...
pub async fn generate_kbs_configurations(
    ...
    let rvps_config = include_str!("rvps.json");
    ...
    for (filename, content, configmap) in [
        ...
        ("rvps.json", rvps_config, &trustee.rvps_configuration),
    ] {
...
pub async fn generate_reference_values(
    ...
    let reference_values = reference_values_in
        ...
        .map(|(name, val)| match val {
            serde_json::Value::String(value) => Ok(reference_values::ReferenceValue {
                name: format!("tpm_{name}"),
                expired: "2099-12-31T23:59:59Z".to_string(),
                hash_value: reference_values::ReferenceHashValue {
...
pub async fn generate_attestation_policy(
    ...
    let mut data = BTreeMap::new();
    data.insert("default.rego".to_string(), policy_rego.to_string());
